...
async function convertAttribute(value, htmlOptions, tag, attr) {
    ...
    if (fetchRemote && value.startsWith("http")) {
        ...
        const base64 = convertFileToBase64(value, verbose, `Embedded asset: ${value} (referenced in <${tag} ${attr}>)\n`, `Failed to embed asset ${value} (referenced in <${tag} ${attr}>)\n`);
        return base64;
    }
    return value;
}
//  Function to convert all HTML element attribute.links from paths to data from http and local files
export default async function convertHTMLLinks(dom, htmlOptions) {
